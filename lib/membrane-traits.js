...
// with in membrane.  
// For details see `PropertyMembrane`.
var Getter = Object.freeze(function membraneGetter(key) {
  return this[key]
})
...
// ### Method ###
// Internal factory function that is used to generate method membranes.
// Function takes `target` object wrapped by a membrane and method `name` as
// arguments and returns a function that proxies to a wrapped one once it's
// called.
// For details see `PropertyMembrane`.
var Method = Object.freeze(function Method(target, key) {
  return Object.freeze(function membraneMethod() {
    // If value of pseudo-variable `this` matches the `membrane` property
    // of wrapped object then `this` should be a wrapped object itself, if not
    // then method was invoked with `apply` or `call` and `this` pseudo variable
    // is just passed through.
    return target[key].apply(target.membrane != this ? this : target, arguments)
  })
})
...
function PropertyMembrane(target, name) {
  ...
  if ('value' in property) {
    ...
    if ('function' === typeof value) {
      descriptor.value = Method(target, name)
